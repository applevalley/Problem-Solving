# 문제를 풀면서 재귀의 개념에 대해 다시 생각해볼 수 있었다.
# 재귀가 중첩될 때마다 숫자를 올려 그 수만큼 언더바 묶음을 붙여주는 형태로 재귀가 얼마나 중첩되는지를 나타낼 수 있었다.
# 복사해온 본문에 쌍따옴표가 있는 곳이 있는데, 백슬래시를 통해 그대로 출력해줄수도 있겠지만 편의상 작은 따옴표로 표현했다.
